...
const EmitterMixin: Emitter = {
	...
	once( event, callback, options ) {
		...
		const onceCallback: Callback = function( event, ...args ) {
	...
	listenTo( emitter, event, callback, options = {} ) {
		let emitterInfo: EmitterInfo, eventCallbacks: Callback[];
	...
	fire( eventOrInfo, ...args ) {
		try {
			...
			if ( callbacks ) {
				...
				const callbackArgs: [EventInfo, ...any[]] = [ eventInfo, ...args ];
...
export interface Emitter {
	...
	[ _listeningTo ]?: {
		[ emitterId: string ]: EmitterInfo;
	};

	_events?: {
		[ eventName: string ]: EventNode;
	};

	_delegations?: Map<string, Map<Emitter, string | ( ( name: string ) => string ) | undefined>>;
}

export type Callback = ( this: Emitter, ev: EventInfo, ...args: any[] ) => void;
 ...
 */
export function _getEmitterListenedTo(
...
function getEvents( source: Emitter ) {
	if ( !source._events ) {
...
function createEventNamespace( source: Emitter, eventName: string ): void {
	...
	let childEventName: string | null = null;

	// Array containing all newly created specific events.
	const newEventNodes: EventNode[] = [];
...
function getCallbacksListsForNamespace(
	source: Emitter,
	eventName: string
): CallbackDefinition[][] {
