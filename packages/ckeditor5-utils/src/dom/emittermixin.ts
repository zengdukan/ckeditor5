...
const DomEmitterMixin: Emitter = extend( {}, EmitterMixin, {
	 ...
	 */
	listenTo( emitter: BaseEmitter | Node | Window, event: string, callback: Callback, options: CallbackOptions = {} ) {
	 ...
	 */
	_getProxyEmitter( node: Node | Window, options: CallbackOptions ): ProxyEmitter | null {
		return _getEmitterListenedTo( this as any, getProxyEmitterId( node, options ) ) as ProxyEmitter | null;
	},
	...
	_getAllProxyEmitters( node: Node | Window ): ProxyEmitter[] {
		...
		].map( options => this._getProxyEmitter( node, options ) ).filter( proxy => !!proxy ) as ProxyEmitter[];
	}
} );
...
class ProxyEmitter {
	 ...
	 * @param {Object} options Additional options.
	 * @param {Boolean} [options.capture=false] Indicates that events of this type will be dispatched to the registered
	 * listener before being dispatched to any EventTarget beneath it in the DOM tree.
	 * @param {Boolean} [options.passive=false] Indicates that the function specified by listener will never call preventDefault()
	 * and prevents blocking browser's main thread by this event handler.
	 */
	constructor( node: Node | Window, options: { capture: boolean, passive: boolean } ) {
	 ...
	 * order they were added.
	 */
	_addEventListener( event: string, callback: Callback, options: CallbackOptions): void {
		this._attach( event );
	 ...
	 */
	_removeEventListener( event: string, callback: Callback ) {
	 ...
	 */
	_createDomListener( event: string ) {
...
function getProxyEmitterId( node: Node | Window, options: CallbackOptions ) {
...
export interface Emitter extends BaseEmitter {
	listenTo<K extends keyof HTMLElementEventMap>(
		emitter: Node | Window,
		event: K,
		callback: ( this: this, info: EventInfo, event: HTMLElementEventMap[K] ) => void,
