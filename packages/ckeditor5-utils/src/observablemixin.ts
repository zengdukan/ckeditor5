...
const ObservableMixin: Observable = {
	 ...
	 * @inheritDoc
	 */
	unbind( ...unbindProperties: unknown[] ) {
		...
		if ( unbindProperties.length ) {
			...
			unbindProperties.forEach( propertyName => {
				...
				// let toProperties;
				// let toPropertyBindings: Record<string, Binding>;

				binding.to.forEach( to => {
					// TODO: ES6 destructuring.
					const toObservable = to[ 0 ];
					const toProperty = to[ 1 ];
	...
	decorate( this: Observable & { [ x: string ]: any }, methodName: string ) {
		...
		this.on( methodName, ( evt: EventInfo, args: unknown[] ) => {
...
ObservableMixin.stopListening = function(
	this: Observable & Record<string, any>,
	emitter,
	event,
	callback
): void {
...
function initObservable( observable: Observable ): asserts observable is InitializedObservable {
...
function bindTo( this: BindChain, ...args: ( Observable | string | Function )[] ): void {
...
function parseBindToArgs( ...args: ( Observable | string | Function )[] ) {
	...
	const parsed: { to: ToBinding[]; callback?: Function; } = { to: [] };
	let lastObservable: ToBinding;
...
function updateBindToBound( chain: BindChain ): void {
	let toProperty: string;
...
function updateBoundObservableProperty(
	...
	if ( binding.callback ) {
		propertyValue = binding.callback.apply( observable, binding.to.map( getValue ) );
	} else {
		propertyValue = getValue( binding.to[ 0 ] );
	}

	const untypedObservable: any = observable;

	if ( Object.prototype.hasOwnProperty.call( observable, propertyName ) ) {
		untypedObservable[ propertyName ] = propertyValue;
	} else {
		untypedObservable.set( propertyName, propertyValue );
	}

	function getValue( [ observable, propertyName ]: [ any, string ] ): unknown {
		return observable[ propertyName ];
	}
}
 ...
 */
// type T = {a: number, b: string, c: number}
export interface Observable extends Emitter {
	set<K extends keyof this & string>( name: K, value: this[K] ): void;
	set( values: { [K in keyof this]?: unknown } ): void;

	bind( ...bindProperties: ( keyof this & string )[] ): BindChain;
	...
	[ boundPropertiesSymbol ]?: BoundPropertiesMap;

	[ boundObservablesSymbol ]?: BoundObservablesMap;

	[ _decoratedMethods ]?: string[];
}
...
type BoundPropertiesMap = Map<string, Binding>;
type BoundObservablesMap = Map<Observable, {
	[ propertyName: string ]: Set<Binding>;
}>;

interface InitializedObservable extends Observable {
	[ observablePropertiesSymbol ]: Map<string, unknown>;
	[ boundPropertiesSymbol ]: BoundPropertiesMap;
	[ boundObservablesSymbol ]: BoundObservablesMap;
}
...
interface BindChain {
	to( ...args: ( Observable | string | Function )[] ): void;

	toMany<O extends Observable, K extends keyof O & string>(
		observables: readonly O[],
		key: K,
		callback: ( ...values: O[K][] ) => void
	): void;
