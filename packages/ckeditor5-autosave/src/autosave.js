...
export default class Autosave extends Plugin {
	...
	init() {
		...
		const users = this.editor.plugins.has( 'Users' ) ? this.editor.plugins.get( 'Users' ) : null;
		...
		this.listenTo( editor, 'ready', () => {
			this.listenTo( doc, 'change:data', ( evt, batch ) => {
				...
				if ( users ) {
					const hasLocalOperation = batch.operations.some( operation => users.getOperationAuthor( operation ) === users.me );
